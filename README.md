...
## Architecture and pricing documentation

Start with [the documentation index](docs/README.md), then [Architecture](docs/ARCHITECTURE.md), [Pricing rules](docs/PRICING_RULES.md), and [Current pricing reference](docs/CURRENT_PRICING.md). The reference can be refreshed with `node --import tsx scripts/export-pricing-reference.ts` after content changes.

Historical implementation plans and original source briefs are preserved in [`plans/`](plans/) as `yyyymmdd-feature-name.md`. They are archival context; [`AGENTS.md`](AGENTS.md) explains how future sessions maintain the live documentation.
...
- [Maintain combos and deals](docs/combos-guide.md) — cards, choice groups, included sizes, upgrades and extras.
...
- Four combo slider cards with eligible variants, upsize prices, inherited benefits, and linked product details.
...
| Combo cards, eligible choices and pricing policies | `src/data/combos.ts` |
| Generic combo eligibility and validation | `src/domain/combos.ts` |
...
See `docs/image-workflow.md` for the maintained photo workflow and `tmp/reports/photo-size-report.json` for local last-run diagnostics. The report is temporary and ignored by Git.
