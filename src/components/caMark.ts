import { splitCaMark } from '../domain/menu';
import { escapeHtml } from '../domain/presentation';
...
// inside the box as in the design. Shared by Localized.astro and the browser
// renderers (combo-view.ts), so the path data lives in exactly one place.
...
// Size and placement come from .ca-mark in global.css (relative to the font size).
// The box follows the surrounding text color; themes may recolor the glyph.
const caMarkSvg = `<svg class="ca-mark" viewBox="0 0 ${CA_MARK_WIDTH} ${CA_MARK_HEIGHT}" aria-hidden="true"><rect width="${CA_MARK_WIDTH}" height="${CA_MARK_HEIGHT}" fill="currentColor"/><g fill="#fff">${CA_MARK_PATHS.map((d) => `<path d="${d}"/>`).join('')}</g></svg>`;
