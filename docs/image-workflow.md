...
The current photo batch produced roughly 5–8 KB thumbnails and 29–54 KB 800px images. Future sizes depend on image detail; there is no fixed byte-size guarantee. Larger detail files load only after opening a product, and extra views load when selected.
...
- `docs/photo-size-report.json`: sizes for the **most recent run**, with `scope` of `all`, `selected`, or `remove`. A selective run's totals are not the whole library's totals.
