...
Copy opts in with the `[cà]` marker; see [Combos guide](combos-guide.md#show-the-cà-logo-mark-in-a-title). Campaign titles show the mark wherever they render (slider card, combo dialog, product combo link). Category names show it only in their section heading: `index.astro` passes the stripped name to the navigation tab, and `menu.ts`, search and the pricing export use `stripCaMark`. A new render site for either field must do one or the other. `.ca-mark` in `global.css` sizes it at `1em`, which matches the glyph's x-height to Be Vietnam Pro, with `vertical-align: -0.14em`: `-0.18em` would put the glyph exactly on the text baseline, and the extra 0.04em lift is an optical correction because the heavy box otherwise reads as sitting low. Revisit these values if the body font changes. The box uses `currentColor` and the glyph is white, except on the `ink` theme (`combos.css`) and the Signature section heading (`global.css`), where the glyph takes the card color. The mark is `aria-hidden`; a `.sr-only` copy of the word keeps it in the accessible name. Both sit in a `position: relative` `.ca-mark-word` span: without it the absolutely positioned copy escapes scroll containers such as the deals slider and widens the page on phones.
...
| `npm run test:e2e` | 34 current desktop/mobile cases, including dialogs, Back, language, slider, horizontal overflow (measured against `visualViewport.width`, because mobile emulation widens `innerWidth` to fit overflowing content), 320px layout, benefits, price labels, the `[cà]` logo mark in combo titles and the Signature heading across languages, and no-JavaScript catalog |
